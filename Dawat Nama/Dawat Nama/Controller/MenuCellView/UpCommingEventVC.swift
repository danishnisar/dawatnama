...
class UpCommingEventVC: UIViewController {
    ...
    var refersh:UIRefreshControl={
        ...
        refreshControl.attributedTitle = NSAttributedString(string: "Fetching Up Comming Events...", attributes: nil)
    ...
    private func networkUpcommingEvents(){
       ProgressHUD.show()
        ...
        Alamofire.request(RestFull.fetchUpcommingEvents, method: .post, parameters: param).responseJSON(completionHandler: { (response) in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "upcommingeventdetail" {
            let data = segue.destination as! EventAgainVC
            print(selectedIndex)
            data.datamigrate.append(upcommingModel[selectedIndex])
            return
        }
...
extension UpCommingEventVC:UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            ...
            print(upcommingModel[indexPath.row].event_name)
    ...
    private func showEvents(){
        performSegue(withIdentifier: "upcommingeventdetail"){
            ProgressHUD.dismiss()
        }
    }
